...
describe WestfieldLabs::DatetimeHelper do
  describe '#is_zulu_time?' do
    context "given a valid Zulu Time string" do
      ...
      it { expect(WestfieldLabs::DatetimeHelper.is_zulu_time?(valid_string)).to eq true }
    end
    ...
    context "given a valid ISO 8601 string that's not Zulu Time" do
      ...
      it { expect(WestfieldLabs::DatetimeHelper.is_zulu_time?(invalid_string)).to eq false }
    end

    context "given nonsense" do
      let(:nonsense) { "not really a zulu time string" }
      it { expect{ Time.parse(nonsense)}.to raise_error }
      it { expect(WestfieldLabs::DatetimeHelper.is_zulu_time?(nonsense)).to eq false }
    end
  end
end
