...
class TestSerialiser < ActiveModel::Serializer
  ...
  attributes :updated_at, :enabled_date, :enabled_date_time

  enforce_zulu_time :updated_at
  enforce_iso8601_date :enabled_date
  enforce_iso8601_date :enabled_date_time
end

class TestItemNoTime
  include ActiveModel::SerializerSupport
  attr_reader :updated_at, :enabled_date, :enabled_date_time
  def initialize(options = {})
    @updated_at = nil
    @enabled_date = nil
    @enabled_date_time = nil
  end
end
...
class TestItem < TestItemNoTime
  def initialize(options = {})
    ...
    @enabled_date_time = options[:enabled_date_time] || Time.now
  end
end
...
describe DatetimeHelper::Serialisers do
  ...
  context 'with a date-time' do
    let(:item)     { TestItem.new }
    it { expect(subject[:enabled_date_time]).to be_an_iso_formatted_date }
  end

  context 'with a nil date-time' do
    let(:item)     { TestItemNoTime.new }
    it { expect(subject[:enabled_date_time]).to be_nil}
  end
